...
public class Principal extends JFrame {
	...
	protected void previousCard() {
		...
		if (cardNumber == 1) {// Panel Atleta Menu
			...
			toOrganizadorMenu();
		} else if (cardNumber == 7) { // Panel Mi Historial
			toAtletaMenu();
		} else if (cardNumber == 8) { // Club Menu
			toFirst();
		}
	...
	private void toRegistrarse() {
		pnButtons.setVisible(true);
		txtNombreAtleta.setText("");
		txtApellidosAtleta.setText("");
		txtEmailAtleta.setText("");
		txtDniAtleta.setText("");
		comboBoxDiaNacimiento.setSelectedIndex(0);
		comboBoxMesNacimiento.setSelectedIndex(0);
		comboBoxAñoNacimiento.setSelectedIndex(0);
		cardNumber = 3;
		...
		try {
			...
			if (correcto) {
				...
				atleta.fechaNacimiento = new Date(
						Integer.valueOf(comboBoxAñoNacimiento.getSelectedItem().toString()) - 1900,
						Integer.valueOf(comboBoxMesNacimiento.getSelectedItem().toString()),
						Integer.valueOf(comboBoxDiaNacimiento.getSelectedItem().toString()));

				registrar.registrar(atleta);
				JOptionPane.showMessageDialog(this, "El registro del nuevo atleta ha sido un �xito");
				HacerInscripcion verAtleta = new HacerInscripcion();
				atleta = verAtleta.getAtleta(atleta.email);
				toAtletaMenu();
			}
	...
	protected void cargarPagos(Competicion competicion) {
		...
		if (file != null) {
			try {
				...
				JOptionPane.showMessageDialog(this, "** Total registro procesados " + total + " ** OKs " + oks
	...
	protected void cargarResultados(Competicion competicion) {
		...
		try {
			...
			JOptionPane.showMessageDialog(getThis(), e.getMessage(), "Fallo en la carga de resultados",
	...
	private void loadResultadosCompeticion(List<Resultados> results) {
		...
		for (int i = 0; i < results.size(); i++) {
			m[i] = new Object[] { results.get(i).getPosicion(), results.get(i).getDorsal(),
					results.get(i).getTiempo() };
		}

		modelInscripcionesResultadosCompeticion.setDataVector(m, new String[] { "Posicion", "Dorsal", "Tiempo" });
	...
	protected void inscribirLote(Competicion competicion) {
		String file = JOptionPane.showInputDialog(this, "Escribe el nombre del fichero con los atletas a inscribir");
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "** Total atletas inscritos " + oks + " ** Total no inscritos " + kos);
	...
	private JPanel getPnErroneos() {
		if (pnErroneos == null) {
			...
			pnErroneos.add(getLblMenuOrganizadorResultados_1(), BorderLayout.NORTH);
	...
	private JLabel getLblMenuOrganizadorResultados_1() {
		if (lblMenuOrganizadorResultados_1 == null) {
			lblMenuOrganizadorResultados_1 = new JLabel("Menu Organizador: Resultados Erroneos");
			lblMenuOrganizadorResultados_1.setFont(new Font("Tahoma", Font.PLAIN, 16));
		}
	...
	private void loadErroneos() {
		...
		for (int i = 0; i < erroneos.size(); i++) {
			m[i] = new Object[] { erroneos.get(i).getDorsal(), erroneos.get(i).getMotivo() };
		}

		modelErroneos.setDataVector(m, new String[] { "Dorsal", "Motivo" });
